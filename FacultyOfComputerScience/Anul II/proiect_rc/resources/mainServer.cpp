...
public:
    ...
    {
        if (id == -1 || id == 2)//daca am castigat sau pierdum iesim din joc
            message = "2" + message;
        else
...
public:
    ...
    void restart_game(bool needs_update = true)
    {
        if (players.size() == 1)
        {
            finished_game = true;
...
//jocurile sunt retinute dupa nume, cand este create un joc acesta este memorat dupa nume
//de exemplu, daca un utilizator vrea sa dea join la un joc, daca acesta nu exista acel joc va fi creat main intai apoi va putea intra si el
