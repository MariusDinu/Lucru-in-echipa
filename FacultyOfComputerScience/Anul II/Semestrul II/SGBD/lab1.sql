...
--Studentii care au promovat
select s.nume,n.valoare INTO val_ro,val2_ro from studenti s join note n on s.id=n.id_student join cursuri c on n.id_curs=c.id where c.titlu_curs like 
( select titlu_curs from ( select c.titlu_curs from profesori p join didactic d on p.id=d.id_profesor join cursuri c on d.id_curs=c.id where p.nume like 'Dinu' group by c.titlu_curs) where rownum=1 )
and n.valoare>4;
END;
 --Studentii care au picat
...
( select titlu_curs from ( select c.titlu_curs from profesori p join didactic d on p.id=d.id_profesor join cursuri c on d.id_curs=c.id where p.nume like '&v_profesor' group by c.titlu_curs) where rownum=1 )
and n.valoare<5;
--Toate materille predate
...
like (select titlu_curs from ( select c.titlu_curs from profesori p join didactic d on p.id=d.id_profesor join cursuri c on d.id_curs=c.id
 where p.nume like '&v_profesor' group by c.titlu_curs) where rownum=1);
