...
TCHAR mappedFileName[] = TEXT("Global\\FisierComun");
		...
		case WAIT_OBJECT_0:
			__try {
				...
				if (A * 2 == B) {
					...
					printf("Esti mai bolund decat prevede legea %d %d numerele fiind %s\n", A, B, numere);
				}
				dwCount++;
			}
			__finally {
				...
				if (!ResetEvent(ghWriteEvent)) {//Incheiem semnalul de write
					printf("T2:Vezi ca nu merge resetu\n");
				}
				if (!SetEvent(ghReadEvent))//Incepem semnalul Read pt a debloca t1
				{
					printf("T1:Vezi ca nu am putut da release\n");
				}
